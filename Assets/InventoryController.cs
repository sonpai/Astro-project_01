...
public class InventoryController : MonoBehaviour
{
    ...
    {
        LoadInventory();
        // Test: Add a sword and berries on start
        ItemData sword = ItemDataManager.Instance.GetItemByID("Sword");
        ItemData berry = ItemDataManager.Instance.GetItemByID("Berry");
        if (sword != null) AddItem(sword, 1);
        if (berry != null) AddItem(berry, 5);
    }
    ...
    {
        // Simple add for this example: find first empty slot
        for (int i = 0; i < inventorySize; i++)
        {
            if (_inventorySlots[i].IsEmpty())
            {
    ...
    // ... (Add SwapItems, RemoveItem, etc. here as needed) ...
