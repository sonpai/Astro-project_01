...
//    [Header("Upgrade Panel")] // Add this header
//    public GameObject upgradePanel;
//    public Button openUpgradePanelButton;
//    public Button backFromUpgradeButton;
//    public Transform upgradeItemsContainer;
//    public GameObject upgradeItemUIPrefab; // Assign your new prefab here
...
//        backFromSellButton?.onClick.AddListener(ShowMainShopChoicesPanel);
//        closeShopOverallButton?.onClick.AddListener(CloseEntireShop);
//        openUpgradePanelButton?.onClick.AddListener(ShowUpgradePanel);
//        backFromUpgradeButton?.onClick.AddListener(ShowMainShopChoicesPanel);
...
//    // Add these methods inside ShopUIManager.cs
...
//        var playerSlots = InventoryController.Instance.InventorySlots_ReadOnly;
//        for (int i = 0; i < playerSlots.Count; i++)
//        {
//            ItemData itemData = InventoryController.Instance.GetItemDataInSlot(i);

//            if (itemData != null && itemData.nextLevelUpgrade != null)
//            {
//                GameObject itemGO = Instantiate(upgradeItemUIPrefab, upgradeItemsContainer);
//                itemGO.GetComponent<UpgradeItemUIController>().Setup(itemData, i);
//            }
//        }
//    }
...
// File: ShopUIManager.cs (Corrected and Final Version)
using UnityEngine;
using UnityEngine.UI;
using TMPro;
using System.Collections.Generic;
...
public class ShopUIManager : MonoBehaviour
{
    ...
    public GameObject buyPanel;
    public GameObject sellPanel;
    public GameObject upgradePanel; // Make sure this is assigned in Inspector

    [Header("Buttons")]
    public Button openBuyPanelButton;
    public Button openSellPanelButton;
    public Button openUpgradePanelButton; // Make sure this is assigned in Inspector
    public Button backFromBuyButton;
    public Button backFromSellButton;
    public Button backFromUpgradeButton; // Make sure this is assigned in Inspector
    public Button closeShopOverallButton;

    [Header("Item List Containers")]
    public Transform buyItemsContainer;
    public Transform sellItemsContainer;
    public Transform upgradeItemsContainer; // Make sure this is assigned in Inspector

    [Header("Item UI Prefabs")]
    public GameObject shopItemUIPrefab;
    public GameObject upgradeItemUIPrefab; // Make sure this is assigned in Inspector
    ...
    void Start()
    {
        ...
        // Subscribe to events for automatic UI updates
        if (PlayerWallet.Instance != null) PlayerWallet.Instance.OnCoinsChanged += UpdateCoinDisplay;
        if (InventoryController.Instance != null) InventoryController.Instance.OnInventoryRefreshed += RefreshPanelsIfActive;
    ...
    public void OpenEntireShop()
    {
        ...
        ShowMainShopChoicesPanel();
        if (PlayerWallet.Instance != null) UpdateCoinDisplay(PlayerWallet.Instance.CurrentCoins);
    }
    ...
    private void PopulateUpgradeList()
    {
        if (upgradeItemsContainer == null || upgradeItemUIPrefab == null || InventoryController.Instance == null) return;

        foreach (Transform child in upgradeItemsContainer) Destroy(child.gameObject);

        // We loop through the inventory using the available methods from InventoryController
        for (int i = 0; i < InventoryController.Instance.inventorySize; i++)
        {
            ItemData itemData = InventoryController.Instance.GetItemDataInSlot(i);

            // Check if the item exists and has a potential upgrade
            if (itemData != null && itemData.nextLevelUpgrade != null)
            {
                GameObject itemGO = Instantiate(upgradeItemUIPrefab, upgradeItemsContainer);
                UpgradeItemUIController uiController = itemGO.GetComponent<UpgradeItemUIController>();
                if (uiController != null)
                {
                    // Pass the item data and its ORIGINAL slot index to the UI
                    uiController.Setup(itemData, i);
                }
            }
        }
    }
    ...
    private void PopulateSellList()
    {
        ...
        // Loop using the correct properties and methods from InventoryController
        for (int i = 0; i < InventoryController.Instance.inventorySize; i++)
        {
            ItemData itemData = InventoryController.Instance.GetItemDataInSlot(i);
            int quantity = InventoryController.Instance.GetQuantityInSlot(i);
            ...
            if (itemData != null && quantity > 0 && itemData.canBeSold && itemData.sellPrice > 0)
            {
                ...
                if (uiController != null)
                {
                    // We now pass the original slot index 'i' so the UI controller knows exactly which stack to sell from
                    // This is more robust than just passing ItemData
                    uiController.SetupSellItem(itemData, quantity, i, _shopSystem);
                }
            }
        }
    }

    private void RefreshPanelsIfActive()
    {
        if (sellPanel != null && sellPanel.activeInHierarchy)
        {
            PopulateSellList();
    ...
    private void UpdateCoinDisplay(int newAmount)
    {
        if (playerCoinsText != null) playerCoinsText.text = $"Coins: {newAmount}";
    }
